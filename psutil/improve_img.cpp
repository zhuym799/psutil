...
namespace psutil
{
	...
	Mat CCadreArchImprove::replace_bgColor(Mat src, Vec3b bgrReplacer, int grayThresh)
	{
		if (src.channels() != 3)
		{
            throw cv::Exception(0,"待编辑图：仅接受BGR三通道的图片。","replace_bgColor","CCadreArchImprove",0);
		}
	...
	Mat CCadreArchImprove::mergeClean(Mat src, Mat binary, Vec3b newbg)
	{
		if (src.channels() != 3)
		{
            throw cv::Exception(0,"待编辑图：仅接受BGR三通道的图片。","mergeClean","CCadreArchImprove",0);
		}
		if (binary.channels() > 1)
		{
            throw cv::Exception(0,"参照图：仅接受灰度图。","mergeClean","CCadreArchImprove",0);
		}
	...
	Mat CCadreArchImprove::setHSV(Mat srcBGR, uchar h, uchar s, uchar v)
	{
		...
		{
            throw cv::Exception(0,"待编辑图：仅接受BGR三通道的图片。","setHSV","CCadreArchImprove",0);
		}
	...
	void CCadreArchImprove::sharpBGR(Mat& srcBGR, const HsvRngBGR bgrRng, const Vec3b standard[3])
	{
		if (srcBGR.channels() != 3)
		{
            throw cv::Exception(0,"待编辑图：仅接受BGR三通道的图片。","sharpBGR","CCadreArchImprove",0);
		}
	...
	void CCadreArchImprove::textfill(Mat& srcBGR, int grayThresh, Size morph_size, Vec3b padding)
	{
		if (srcBGR.channels() != 3)
		{
            throw cv::Exception(0,"待编辑图：仅接受BGR三通道的图片。","textfill","CCadreArchImprove",0);
		}
	...
	void CCadreArchImprove::partitionCopyPaste(Mat & srcBGR, Mat refMat, Rect refRng, Point2i target)
	{
		if (srcBGR.channels() != 3)
		{
            throw cv::Exception(0,"待编辑图：仅接受BGR三通道的图片。","partitionCopyPaste","CCadreArchImprove",0);
		}
		if (refMat.channels() != 3)
		{
            throw cv::Exception(0,"参照图：仅接受BGR三通道的图片。","partitionCopyPaste","CCadreArchImprove",0);
		}
	...
	void CCadreArchImprove::replaceColors(Mat& srcBGR, Rect range, Vec3b (* colors)[2])
	{
		if (srcBGR.channels() != 3)
		{
            throw cv::Exception(0,"待编辑图：仅接受BGR三通道的图片。","replaceColors","CCadreArchImprove",0);
		}
