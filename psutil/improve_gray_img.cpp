...
namespace psutil
{
	...
	*/
	float CImproveGray::getAngleByHough(Mat src, int grayThresh, int houghVote, int tolerant)
	{
		...
		{
            throw cv::Exception(0,"二值化阈值的取值范围为：0~255","getAngleByHough","CImproveGray",0);
		}
		float angle = 0;
		...
		if (qty > 0)
		{
			...
			if (thetas.size() > 0)
			{
                ...
                if (angle != 0 && angle != static_cast<float>(CV_PI / 2))
				{
...
//				//imwrite("D:\\testResult\\drawContours" + to_string(blurSize.width) + ".jpg", drawing);
	...
	float CImproveGray::findThetaFromPerpendiculars(vector<float> lineThetas, int tolerant)
	{
		...
		if (cnt > 0)
		{
            ...
            theta = theta < degree90 ? theta : static_cast<float>(theta - CV_PI);
		}
		return theta;
	}
	...
	float CImproveGray::findThetaFromObliques(vector<float> lineThetas, int tolerant)
	{
		float theta = 0;
		size_t cnt = 0; // cnt of thetas 
        float tolerantPi = static_cast<float>(tolerant * CV_PI / 180); // 允许的偏差
		for (size_t i = 0; i < lineThetas.size(); i++)
		{
            if (abs(lineThetas[i]) <= tolerantPi || static_cast<float>(abs(lineThetas[i] - CV_PI / 2) <= tolerantPi))
			{
				continue;
			}
			theta += lineThetas[i];
		...
		{
			theta = theta / cnt;
            theta = theta < static_cast<float>(CV_PI / 2) ? theta : static_cast<float>(theta - CV_PI);
		}
		return theta;
	}
	...
	*/
	...
	Mat CImproveGray::replaceColors(Mat srcBin, Rect range, Vec3b(*colors)[2])
	{
		...
		{
            throw cv::Exception(0,"待编辑图：仅接受单通道的二值图片。","replaceColors","CImproveGray",0);
		}
		Mat dest;
